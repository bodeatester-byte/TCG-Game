...
socket.on('authResponse', (data) => {
    ...
    if (data.success) {
        myUsername = data.username; localStorage.setItem('tcg_username', myUsername);
        document.getElementById('auth-container').style.display = 'none';
        if (data.inGame && data.roomId) {
            myRole = data.role; currentRoomId = data.roomId;
            document.getElementById('lobby-container').style.display = 'none';
...
if (document.getElementById('find-game-btn')) {
    document.getElementById('find-game-btn').onclick = () => {
        ...
        document.getElementById('find-game-btn').innerText = "Se caută...";
...
socket.on('updateGameState', (state) => {
    ...
    updateUI(); afiseazaBannerTura(activePlayer);
});
...
function updateUI() {
    ...
    if (p2h && p2Hand) {
        p2h.innerHTML = ''; p2Hand.forEach((card, idx) => {
            ...
            } else { el.style.backgroundColor = '#444'; el.style.color = '#888'; el.innerHTML = `<br><br><br><b>TCG</b>`; }
            p2h.appendChild(el);
        });
    }
// =========================================================================
...
socket.on('updateLeaderboard', (list) => {
    ...
    list.forEach((p, idx) => {
        ...
        r.innerHTML = `<td style="color:${rc}; font-weight:bold; padding:6px;">#${idx + 1}</td><td style="padding:6px;">${p.username}</td><td style="text-align:right; padding:6px;">${p.wins}</td>`; b.appendChild(r);
    });
});
socket.on('playerDisconnectedWaiting', (data) => { if(document.getElementById('game-message')) document.getElementById('game-message').innerText = `⚠️ Jucătorul ${data.username} s-a deconectat! Are 15 secunde să revină.`; });
updateUI();
